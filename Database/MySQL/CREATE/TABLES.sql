...
CREATE TABLE IF NOT EXISTS rpt.ILOGELT_INSIGHTS(
   ...
   ,LOG_ARC_VER CHAR(12)
   ,LOG_GW_VER VARCHAR(256) -- INT 
   ,LOG_LANG_TE VARCHAR(256)
   ,LOG_LANG_NR INT  
   ,LOG_REC_TE NVARCHAR(256)
   ,LOG_STR_DT DATETIME -- VARCHAR(256)
   ,LOG_END_DT DATETIME -- VARCHAR(256)
...
CREATE TABLE IF NOT EXISTS rpt.IRPTROT_ROTATION (
    LOG_SYS_NR CHAR(36)
   ,LOG_ROT_ID INT
   ,LOG_ROT_ACC NVARCHAR(256)
   ,LOG_ROT_NA NVARCHAR(256)
   ,LOG_ROT_CT INT
   ,LOG_ROT_DUR INT
   ,LOG_ROT_GN INT
   ,LOG_ROT_QK FLOAT
   ,PRIMARY KEY(LOG_SYS_NR, LOG_ROT_ID, LOG_ROT_ACC, LOG_ROT_CT)
   ,FOREIGN KEY(LOG_SYS_NR) REFERENCES log.TLOGCAT(LOG_SYS_NR) ON DELETE CASCADE
);
*/
