...
CREATE PROCEDURE web.postRotations(jsonObject JSON)
BEGIN
	WITH Rotations_CTE AS (
		SELECT ELT.LOG_SYS_NR, ELT.LOG_FGT_NA, ELT.LOG_FGT_IC, ELT.LOG_STR_DT, LOG_ACC_NA, LOG_CHR_NA, LOG_PRO_NA, ROT.LOG_SKL_ID, ROT.LOG_CST_NR, ROT.LOG_DUR_NR
		FROM rpt.IRPTPLY_PLAYERS PLY 
		INNER JOIN rpt.IRPTROT_ROTATION ROT
			ON PLY.LOG_SYS_NR = ROT.LOG_SYS_NR
			AND PLY.LOG_CHR_NA = ROT.LOG_ACT_NA
		INNER JOIN rpt.ILOGELT_INSIGHTS ELT
			ON ROT.LOG_SYS_NR = ELT.LOG_SYS_NR
	)
	SELECT A01.LOG_SYS_NR, LOG_FGT_NA, LOG_FGT_IC, LOG_STR_DT, LOG_ACC_NA, LOG_CHR_NA, LOG_PRO_NA, LOG_SKL_ID, LOG_CST_NR, LOG_DUR_NR
	FROM Rotations_CTE A01
	INNER JOIN JSON_TABLE(jsonObject->>'$.logs'
		, '$[*]' COLUMNS(
			LOG_SYS_NR CHAR(36) PATH '$.id'
		)) LOGS
...
CREATE PROCEDURE web.getRotations()
BEGIN
	WITH Rotations_CTE AS (
		SELECT ELT.LOG_SYS_NR, ELT.LOG_FGT_NA, ELT.LOG_FGT_IC, ELT.LOG_STR_DT, LOG_ACC_NA, LOG_CHR_NA, LOG_PRO_NA, ROT.LOG_SKL_ID, ROT.LOG_CST_NR, ROT.LOG_DUR_NR
		FROM rpt.IRPTPLY_PLAYERS PLY 
